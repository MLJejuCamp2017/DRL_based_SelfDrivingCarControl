...
public class Vehicle7_Manager : MonoBehaviour
{
...
//			}
			...
			if (vehicle7_obj.Length < Num_vehicles && IsOthers == false) {
				// Set spawn position
				Vector3 spawn_position = SpawnVector;
				GameObject spawned_vehicle = Instantiate (vehicle7, spawn_position, Quaternion.identity);		
				spawned_vehicle.transform.parent = Agent.transform;	
			}

		}
        if (vehicle7_obj != null)
        {
            if (vehicle7_obj.Length > Num_vehicles)
            {
                Destroy(vehicle7_obj[0]);
            }
        }
    }
