...
public class Vehicle3_Manager : MonoBehaviour
{
...
//			}
			...
			if (vehicle3_obj.Length < Num_vehicles && IsOthers == false) {
				// Set spawn position
				Vector3 spawn_position = SpawnVector;
				GameObject spawned_vehicle = Instantiate (vehicle3, spawn_position, Quaternion.identity);		
				spawned_vehicle.transform.parent = Agent.transform;	
			}

		}

        if (vehicle3_obj != null)
        {
            if (vehicle3_obj.Length > Num_vehicles)
            {
                Destroy(vehicle3_obj[0]);
            }
        }
    }
