...
public class Vehicle2_Manager : MonoBehaviour
{
...
//			}
			...
			if (vehicle2_obj.Length < Num_vehicles && IsOthers == false) {
				// Set spawn position
				Vector3 spawn_position = SpawnVector;
				GameObject spawned_vehicle = Instantiate (vehicle2, spawn_position, Quaternion.identity);		
				spawned_vehicle.transform.parent = Agent.transform;		
			}

		}

        if (vehicle2_obj != null)
        {
            if (vehicle2_obj.Length > Num_vehicles)
            {
                Destroy(vehicle2_obj[0]);
            }
        }
    }
