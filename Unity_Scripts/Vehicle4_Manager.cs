...
public class Vehicle4_Manager : MonoBehaviour
{
...
//			}
			...
			if (vehicle4_obj.Length < Num_vehicles && IsOthers == false) {
				// Set spawn position
				Vector3 spawn_position = SpawnVector;
				GameObject spawned_vehicle = Instantiate (vehicle4, spawn_position, Quaternion.identity);		
				spawned_vehicle.transform.parent = Agent.transform;	
			}

		}

        if (vehicle4_obj != null)
        {
            if (vehicle4_obj.Length > Num_vehicles)
            {
                Destroy(vehicle4_obj[0]);
            }
        }
    }
