...
public class Vehicle5_Manager : MonoBehaviour
{
...
//			}
			...
			if (vehicle5_obj.Length < Num_vehicles && IsOthers == false) {
				// Set spawn position
				Vector3 spawn_position = SpawnVector;
				GameObject spawned_vehicle = Instantiate (vehicle5, spawn_position, Quaternion.identity);		
				spawned_vehicle.transform.parent = Agent.transform;	
			}

		}
        if (vehicle5_obj != null)
        {
            if (vehicle5_obj.Length > Num_vehicles)
            {
                Destroy(vehicle5_obj[0]);
            }
        }
    }
