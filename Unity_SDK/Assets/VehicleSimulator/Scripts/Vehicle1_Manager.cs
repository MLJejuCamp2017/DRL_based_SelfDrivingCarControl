...
public class Vehicle1_Manager : MonoBehaviour
{
...
//			float x_host = spawnPoints[0].position.x;
			...
			if (vehicle1_obj.Length < Num_vehicles && IsOthers == false) {
				// Set spawn position
				Vector3 spawn_position = SpawnVector;
				GameObject spawned_vehicle = Instantiate (vehicle1, spawn_position, Quaternion.identity);		
				spawned_vehicle.transform.parent = Agent.transform;
			}

		}

        if (vehicle1_obj != null)
        {
            if (vehicle1_obj.Length > Num_vehicles)
            {
                Destroy(vehicle1_obj[0]);
            }
        }
